...
export default function App() {
  ...
  const getShiftCellStyles = (shift: ShiftType, isToday: boolean) => {
    if (isToday) {
      // Blue highlighted today circle layout
      return {
        bg: 'bg-blue-50 border-2 border-blue-500 shadow-md shadow-blue-100',
        text: 'text-blue-700 font-bold italic text-base',
      };
    }
              ...
              {cells.map((cell, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Cute micro dot for today to improve accessibility */}
                    {isToday && (
                      <span className="absolute bottom-1 w-1.5 h-1.5 rounded-full bg-blue-500 animate-ping" />
                    )}
                  </button>
                );
              })}
